  ...
  addEventListener('DOMContentLoaded', async () => {
    ...
    const render = () => {
      const rows = Object.entries(SOURCES).map(([k, src]) => {
        ...
        return `<li><span class="n"><a href="${src.page}">${src.name}</a></span><span class="s ${l && !l.ok ? 'bad' : ''}">${status}</span>${l ? `<button class="go" type="button" data-sync="${k}">Sync now</button>` : `<a class="go" href="${src.page}">Set up</a>`}</li>`;
      });
      pop.innerHTML = `<h4>Syncs</h4><ul>${rows.join('')}</ul>`;
      pop.querySelectorAll('[data-sync]').forEach(b => b.addEventListener('click', async () => {
        b.textContent = 'Syncing…'; const r = await store.sync(b.dataset.sync); if (r) ledger[b.dataset.sync] = r; paint(); render();
